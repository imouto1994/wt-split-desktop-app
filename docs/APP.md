...
**SharpFactory pattern (no `Sharp.clone()` on the hot path)**: `writeSlices` and `extractEdgeStrip` both take a `SharpFactory = () => sharp.Sharp` function instead of an existing Sharp instance + `.clone()`. The reason: `Sharp.prototype.clone()` internally calls `structuredClone(this.options)`, which throws `DataCloneError: ArrayBuffer could not be cloned` on Windows + Node 22+ for our large pool-backed `Buffer.allocUnsafe(...)` output (see [nodejs/node#55593](https://github.com/nodejs/node/issues/55593) and [Sharp #1660](https://github.com/lovell/sharp/issues/1660) — Node marks pool-backed Buffers as untransferable). Each `factory()` call constructs a fresh Sharp pipeline holding a *reference* to the same underlying input — zero data copy, no `structuredClone`, no `DataCloneError`. Call sites that conceptually want "the same pipeline used multiple times" use this pattern unconditionally so the in-memory path (where the bug fires) and the file-path path (where it doesn't) share the same architecture.
